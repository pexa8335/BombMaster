...
/*                    player.KillCount += killCount;
        ...
        private void HandleWinner(PlayerTank player)
        {
            var lobby = FindLobbyByPlayer(player);
            if (lobby != null)
            {
                // Gửi thông báo GAMEOVER đến tất cả người chơi trong lobby
                foreach (var _player in lobby.Players)
                {
                    SendMessageToPlayer(_player, "GAMEOVER");
                }
            }
        }
        ...
        private void CheckGameOver(PlayerTank player)
        {
            ...
            if (lobby != null)
            {
                ...
                // Gửi thông báo cho tất cả players
                foreach (var p in lobby.Players)
                {
                    string GameOverInfo = $"GAMEOVER;{check}";
                    SendMessageToPlayer(p, GameOverInfo);
                }
            }
        }
